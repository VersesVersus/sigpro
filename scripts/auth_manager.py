...
def generate() -> str:
    ...
    state = {"code": code, "expires_at": int(time.time()) + EXPIRY_SEC}
    save_state(state)
    return code


def validate(submitted_code: str) -> tuple[bool, str]:
    state = load_state()
    if not state:
...
def usage() -> None:
    print("Usage: auth_manager.py generate | validate <code>")
...
if __name__ == "__main__":
    ...
    if cmd == "validate":
        if len(sys.argv) < 3:
            print(json.dumps({"ok": False, "message": "Missing code argument."}))
            sys.exit(1)
